...
public class HttpClientService {
	...
	private static HttpClient client = HttpPoolClient.createDefaultClient();
	 ...
	 * @param reqURL  请求地址(含参数)
	 * @param decodeCharset  解码字符集,解析响应数据时用之,其为null时默认采用UTF-8解码
	 * @param  httpParamMap  httpClient参数设置  key定义在  CoreConnectionPNames
	 * @return 远程主机响应正文
	 */
	public static String sendGetRequest(String reqURL, String decodeCharset,Map<String,Object> httpParamMap) {
		String responseContent = null; // 响应内容
		HttpGet httpGet = new HttpGet(reqURL); // 创建org.apache.http.client.methods.HttpGet
		HttpEntity entity = null;
		try {
			for(String key:httpParamMap.keySet()){
				client.getParams().setParameter(key,httpParamMap.get(key));
			}
			HttpResponse response = client.execute(httpGet); // 执行GET请求
			System.out.println(response);
			entity = response.getEntity(); // 获取响应实体
